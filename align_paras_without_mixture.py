...
def get_aligned_paras_1(id,paras_orig,paras_pred,paras_en):
    alignd_paras_zh = []
    alignd_paras_en = []
    ...
    for i in range(len_pred):
        ...
        for j in range(len_orig):
            ...
            gleu_f = get_gleu(para_pred[0],para_orig[0])
            gleu_l = get_gleu(para_pred[-1],para_orig[-1])
            if not (gleu_f + gleu_l > 0.1):
                    continue
            else:
                if gleu_f + gleu_l > max_gleu:
...
def save_miss():
    try:
        with open(opt.output_dir+'/miss_ids_align_paras.txt', 'wb') as fp:
            for id in miss_ids:
                fp.write(id)
                fp.write('\n')
    except IOError:
...
def main():
    ...
    for id in ids:
        ...
        count_orig += len(paras_en)
        sents_orig = get_num_sent(paras_orig)
        sents_en = get_num_sent(paras_en)
        ret = get_aligned_paras_1(id,paras_orig,paras_pred,paras_en)
        if ret:
            #print id
            alignd_paras_zh,alignd_paras_en = ret
            count += len(alignd_paras_zh)
            sents_align = get_num_sent(alignd_paras_en)
            save_paras(id,alignd_paras_zh,alignd_paras_en)
            print id
