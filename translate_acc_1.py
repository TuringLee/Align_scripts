...
pattern_en_s = re.compile(u'^[^a-zA-Z0-9\']*')
...
def get_input_path(id):
    en_path = os.path.join(opt.input_dir,str(id)+'_en.txt')
...
def main():
    ...
    for i_,id in enumerate(ids):
        ...
        for i,para in enumerate(en_paras):
            ...
            for j in range(len(sents)):
                tmp = re.sub(pattern_en_s,'',sents[j])
                if not tmp:
                    continue
                EN_DICT[(id,i,j)] = len(en_all_sents)
                en_all_sents.append(tmp)
    ...
    for sent in en_all_sents:
        en_sent_length.append(len(sent.split(' ')))
